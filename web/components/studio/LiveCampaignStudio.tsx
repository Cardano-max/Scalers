 ...
 */
import type { ChatTurn, PlanDoc, StudioRole } from '@/lib/data/studio-adapter';
...
/** Map an agent_runs role to a chat persona (role + distinct label). Label-first
 *  resolution in persona.ts colours each as its own card. */
function stepPersona(role: string): { role: StudioRole; label: string } {
...
/** A short, honest preview line for a landed step (the real persisted card with the
 *  full summary replaces it once the run completes and history refreshes). */
function stepText(step: RunStep): string {
  ...
  return o == null ? '' : String(o).slice(0, 200);
}

/** Synthesize live per-agent cards from the polled run steps, so the cards fill in
 *  REAL TIME as each agent lands (before the persisted history refresh at completion). */
function liveRunTurns(runState: RunState | null): ChatTurn[] {
  if (!runState) return [];
  return runState.steps.map((s) => {
    const p = stepPersona(s.role);
    return {
      id: `runstep_${runState.runId || 'live'}_${s.seq}`,
      role: p.role,
      label: p.label,
      text: stepText(s) || `${p.label} working…`,
...
export function LiveCampaignStudio({ aguiUrl, graphqlUrl, sessionId }: LiveCampaignStudioProps) {
  ...
  // While a live run is in flight, append synthesized per-agent cards from the polled
  // run steps so the conversation fills in real time. On completion runningCampaign
  // flips false and the persisted history (refreshed by the hook) takes over — no dupes.
  const liveTurns = studio.runningCampaign ? liveRunTurns(studio.runState) : [];
  const displayTurns: ChatTurn[] = liveTurns.length ? [...studio.turns, ...liveTurns] : studio.turns;
