...
- **PR #124** (draft, branch `claude/supervisor-state-recovery-4mp2vg` → main): all of the above. CI round 1: gateway ✅, web in progress, engine lint ❌ (ruff — FIXED locally), engine integration ❌ (bare DB, no initdb — FIXED via ci.yml step; plus `_research_enabled` monkeypatch fakes and the 4hj hold-lift test fix).
- Operator's standing orders: (a) **merge all PRs** once CI is green (main is branch-protected: 5 required checks, enforce_admins on); (b) keep THIS handoff file updated; (c) operator will supply Meta page token + real artwork images "in a while".
