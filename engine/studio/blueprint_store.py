...
(P1.5 blueprint #1 — the authored plan the planner builds BEFORE drafting).

The blueprint is an AUTHORED plan (not a derived board): one row per run, keyed to
``run_id``, upserted once at plan time and again if the replan flips the assumption. This
is the dedicated durable blueprint row (distinct from the progress BOARD, which is
computed on-demand with NO table per the run-state design). The blueprint is ALSO carried
in the planner ``agent_run.output`` for the trace/lane; this store is the clean keyed
lookup the run endpoint reads.
...
* ``planner_model``— the model tier the planner actually ran at (honest; ``grounded_rules``
                     when the plan was built deterministically with no model call).
...
def upsert_blueprint(
    ...
    *,
    ...
    Best-effort at the call site (never breaks a real run)."""
    setup(dsn)
    with _connect(dsn) as conn:
        conn.execute(
            """
