...
VOICE_INSTRUCTIONS = (
    ...
    "THAN ONE channel (Instagram + email + SMS), interview ONE CHANNEL AT A TIME — "
    "announce it ('Now the Instagram questions…'), finish that channel's block, "
    "then move to the next; each channel gets its OWN goal, audience and count "
    "(Instagram questions are not email questions). Store every per-channel answer "
    "via update_plan channel_plans.{channel}.{field} (channels: ig / email / sms). "
    "The Instagram block MUST also ask: should I research competitor posts first "
    "and mold the best-performing structure to your brand? — attach artwork images "
    "to the post? — and what style/type of images (e.g. fine-line botanical, "
    "black-and-grey realism, healed photos). Email asks its own offer/ask; SMS "
    "copy stays short and compliant, opt-out kept.\n"
