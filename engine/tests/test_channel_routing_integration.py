...
runs and with what archetype (the value the spine writes into the trace):
  ...
  * 'Facebook campaign' / 'artist with attachments' → an HONEST not-built response:
...
def _spy_pipelines(monkeypatch):
    ...
    def _fake_run_and_trace(**kw):
        ...
        chans = ["ig", "reels", "email"] if aid == "artist_spotlight" else ["email"]
        return {
...
def test_facebook_campaign_is_honest_not_built_no_run(monkeypatch):
    calls = _spy_pipelines(monkeypatch)
    plan = CampaignPlan(
        goal="create a Facebook campaign for the holiday promo",
        audience="all",
        channels=["facebook"],
    )
    summary = _execute_campaign_sync(plan, "sess", "t", None, run_id="team-camp_x-abc")

    # NEITHER pipeline ran — no fake run.
    ...
    assert "isn't built yet" in summary["message"]
