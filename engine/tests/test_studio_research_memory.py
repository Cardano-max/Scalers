...
def test_ingest_leads_idempotent_on_seeded() -> None:
    rows = [{"name": SEED_NAME, "email": SEED_EMAIL, "location": "Brooklyn, NY",
             "interests": "floral;color"}]
    res = ingest_leads(TENANT, rows, dsn=DSN)
    assert res["created"] == 0  # already seeded → matched, never duplicated
...
def test_research_and_stage_writes_pending_drafts_and_memory(monkeypatch) -> None:
    ...
    summary = _research_and_stage_sync(
        plan, sid, TENANT, DSN, emails=[SEED_EMAIL], limit=10
    )
