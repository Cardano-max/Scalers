...
def test_text_post_passes_its_media_gate():
    # A text-only post (no media asset, no hashtags) clears the whole bank,
    # incl. media_valid (TEXT => no aspect_ratio / no duration).
    draft = _draft(
        "Booked through spring. Consults are open for summer now.",
        hashtags=[],
        media=_media("text", None, None),
    )
    res = _validators().check(PostDraft.model_validate(draft))
    assert res.ok, res.summary()
    assert not any(i.validator == "media_valid" for i in res.errors)
