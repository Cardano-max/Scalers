...
| anthropics/skills `pptx` *(1mk.8 scout)* | `anthropics/skills` — `skills/pptx` @ `35414756ca55738e050562e272a6bbc6273aa926` (**license: Proprietary**) | **REJECTED-for-vendoring** (sec) | License Proprietary → not redistributable/vendorable. Heavy exec: `office/soffice.py` `subprocess` shell-out to LibreOffice + Node `pptxgenjs` + write-and-run model. | — | — | none | **REJECTED** — mine the idea only; reporting built ORIGINAL (see 1mk.8). First-party use only inside Claude's managed runtime, never vendored. |
| anthropics/skills `pdf` *(1mk.8 scout)* | `anthropics/skills` — `skills/pdf` @ `35414756ca55738e050562e272a6bbc6273aa926` (**license: Proprietary**) | **REJECTED-for-vendoring** (sec) | License Proprietary → not vendorable. Ships form/exec scripts (fill/convert/render) — file-exec surface. | — | — | none | **REJECTED** — mine the idea only; reporting built ORIGINAL (1mk.8). |
| anthropics/skills `docx` *(1mk.8 scout)* | `anthropics/skills` — `skills/docx` @ `35414756ca55738e050562e272a6bbc6273aa926` (**license: Proprietary**) | **REJECTED-for-vendoring** (sec) | License Proprietary → not vendorable. Heavy exec: `office/soffice.py` `subprocess` (LibreOffice) + pack/unpack/validate scripts. | — | — | none | **REJECTED** — mine the idea only; reporting built ORIGINAL (1mk.8). |
| anthropics/skills `xlsx` *(1mk.8 scout)* | `anthropics/skills` — `skills/xlsx` @ `35414756ca55738e050562e272a6bbc6273aa926` (**license: Proprietary**) | **REJECTED-for-vendoring** (sec) | License Proprietary → not vendorable. Ships `office/` exec helpers (soffice/pack/validate) — file-exec surface. | — | — | none | **REJECTED** — mine the idea only; reporting built ORIGINAL (1mk.8). |
...
| **customer-psychology** *(P1.5 scaffold)* | ORIGINAL — first-party wrapper of `engine/studio/psych_profile.py` (no upstream code vendored) @ `c15864bee7a4f0a8975bf179782b99e05ed15867` (real trunk pin; matches the pack `SKILL.md` `pinned:`) | **PENDING** — sec sign-off NOT recorded (sec owns this flip in a separate pass) | **Nothing vendored** — thin progressive-disclosure loader around our OWN already-live, already-tested analyst (13 passing tests); no net/file/exec introduced (`loader.py` imports only `json`/`os` + `studio.psych_profile`) | `engine/studio/skillpacks/customer_psychology/` (SKILL.md + manifest.json + loader.py) | **PENDING** — no gold-set; the analyst's 13 unit tests are its floor | *(intended: analyst — lead classification, progressive-disclosure)* | **IN-VETTING — PENDING sec sign-off** (NOT registered; the live run still imports `psych_profile.py` DIRECTLY — this pack is a scaffold and is NOT a live dependency) |
...
> sign-off / eval-gate / operator adoption — sec owns those. The live system does not depend
> on the pack (the analyst is imported directly), so `scripts/check_skill_registry.py`
> continues to pass with it present.
